...
Builtin parse_builtin_node(Arena *arena, Token_Arr *tokens, Builtin_Type type, Blocks *blocks, Nodes *variables, Functions *functions, Nodes *structs) {
    Builtin builtin = {
        .type = type,
    };
    ADA_APPEND(arena, &builtin.value, parse_expr(arena, tokens, blocks, variables, functions, structs));
...
Expr *parse_primary(Arena *arena, Token_Arr *tokens, Blocks *blocks, Nodes *variables, Functions *functions, Nodes *structs) {
    Token token = token_consume(tokens);
    ...
    switch(token.type) {
        ...
        case TT_CHAR_LIT:
            *expr = (Expr){
            ...
            break;
        case TT_BUILTIN: {
            Builtin value = parse_builtin_node(arena, tokens, token.value.builtin, blocks, variables, functions, structs);
            *expr = (Expr) {
                .type = EXPR_BUILTIN,
                .value.builtin = value,
        ...
        case TT_O_PAREN:
            expr = parse_expr(arena, tokens, blocks, variables, functions, structs);
            ...
            if(token_peek(tokens, 0).type == TT_O_PAREN) {
                *expr = (Expr){
                ...
                while(tokens->count > 0 && token_consume(tokens).type != TT_C_PAREN) {
                    Expr *arg = parse_expr(arena, tokens, blocks, variables, functions, structs);
                ...
                *expr = (Expr){
                ...
                expr->value.array.index = parse_expr(arena, tokens, blocks, variables, functions, structs);
...
Expr *parse_expr_1(Arena *arena, Token_Arr *tokens, Expr *lhs, Precedence min_precedence, Blocks *blocks, Nodes *variables, Functions *functions, Nodes *structs) {
    ...
    while(op_get_prec(lookahead.type) >= min_precedence) {
        ...
        if(tokens->count > 0) {
            token_consume(tokens);
            Expr *rhs = parse_primary(arena, tokens, blocks, variables, functions, structs);
            lookahead = token_peek(tokens, 0);
            while(op_get_prec(lookahead.type) > op.precedence) {
                rhs = parse_expr_1(arena, tokens, rhs, op.precedence+1, blocks, variables, functions, structs);
                lookahead = token_peek(tokens, 0);
            }
...
Expr *parse_expr(Arena *arena, Token_Arr *tokens, Blocks *blocks, Nodes *variables, Functions *functions, Nodes *structs) {
    return parse_expr_1(arena, tokens, parse_primary(arena, tokens, blocks, variables, functions, structs), 1, blocks, variables, functions, structs);
}
...
Node parse_native_node(Arena *arena, Token_Arr *tokens, int native_value, Blocks *blocks, Nodes *variables, Functions *functions, Nodes *structs) {
    Node node = {.type = TYPE_NATIVE, .loc = tokens->data[0].loc};            
    token_consume(tokens);
    Native_Call call = {0};
    Arg arg = {0};
    arg = (Arg){.type=ARG_EXPR, .value.expr=parse_expr(arena, tokens, blocks, variables, functions, structs)};
...
Node parse_var_dec(Arena *arena, Token_Arr *tokens, Blocks *blocks, Nodes *variables, Functions *functions, Nodes *structs) {
    Node node = {0};
    ...
    if(token_peek(tokens, 1).type == TT_O_BRACKET) {
        token_consume(tokens);
        token_consume(tokens);
        node.value.var.array_s = parse_expr(arena, tokens, blocks, variables, functions, structs);       
...
int parse_reassign_left(Token_Arr *tokens, Node *node, Blocks *blocks, Nodes *variables, Functions *functions, Nodes *structs, Arena *arena) {
			Token token = token_peek(tokens, 1);
				if(token.type == TT_EQ) {
                    ...
                    token_consume(tokens); // open bracket                        
                    node->value.array.index = parse_expr(arena, tokens, blocks, variables, functions, structs);
...
Program parse(Arena *arena, Token_Arr tokens, Blocks *block_stack) {
    ...
    while(tokens.count > 0) {
        ...
        switch(tokens.data[0].type) {
            case TT_WRITE: {
                node = parse_native_node(arena, &tokens, NATIVE_WRITE, block_stack, &vars, &functions, &structs);            
                ADA_APPEND(arena, &root, node);
            } break;
            case TT_EXIT: {
                node = parse_native_node(arena, &tokens, NATIVE_EXIT, block_stack, &vars, &functions, &structs);
                ADA_APPEND(arena, &root, node);
            } break;
            case TT_IDENT: {
                Token token = token_peek(&tokens, 1);
                if(token.type == TT_COLON) {
                    node = parse_var_dec(arena, &tokens, block_stack, &vars, &functions, &structs);
                    token_consume(&tokens);						
                    ...
                    if(node.value.var.is_array && node.value.var.type != TYPE_STR) {
                        ...
                        while(tokens.count > 0) {
                            ADA_APPEND(arena, &node.value.var.value, parse_expr(arena, &tokens, block_stack, &vars, &functions, &structs));
                        ...
                        while(tokens.count > 0 && token_peek(&tokens, 0).type != TT_C_CURLY) {
							...
							expect_token(&tokens, TT_EQ);
                            arg.value.expr = parse_expr(arena, &tokens, block_stack, &vars, &functions, &structs);
					...
					int i = parse_reassign_left(&tokens, &node, block_stack, &vars, &functions, &structs, arena);
					if(node.type == TYPE_VAR_REASSIGN) {
	                    ADA_APPEND(arena, &node.value.var.value, parse_expr(arena, &tokens, block_stack, &vars, &functions, &structs));
	                    expr_type_check(node.loc, node.value.var.value.data[node.value.var.value.count-1]);
	                } else if(node.type == TYPE_FIELD_REASSIGN) {
	                    expect_token(&tokens, TT_EQ);
	                    ADA_APPEND(arena, &node.value.field.value, parse_expr(arena, &tokens, block_stack, &vars, &functions, &structs));
	                    expr_type_check(node.loc, node.value.field.value.data[node.value.field.value.count-1]);
	                } else if(node.type == TYPE_ARR_INDEX) {
						Token token = token_peek(&tokens, 0);
						if(token.type == TT_EQ) {
							token_consume(&tokens);
		                    ADA_APPEND(arena, &node.value.array.value, parse_expr(arena, &tokens, block_stack, &vars, &functions, &structs));
	                    ...
	                    token_consume(&tokens);                        
	                    while(tokens.count > 0 && token_consume(&tokens).type != TT_C_PAREN && i > 2) {
	                        Node arg = parse_var_dec(arena, &tokens, block_stack, &vars, &functions, &structs);
            ...
            case TT_RET: {
                node.type = TYPE_RET;
                token_consume(&tokens);
                node.value.expr = parse_expr(arena, &tokens, block_stack, &vars, &functions, &structs);
            ...
            case TT_IF: {
                ...
                token_consume(&tokens);
                node.value.conditional = parse_expr(arena, &tokens, block_stack, &vars, &functions, &structs);
            ...
            case TT_BUILTIN: {
                ...
                node.value.expr_stmt = parse_expr(arena, &tokens, block_stack, &vars, &functions, &structs);
                ADA_APPEND(arena, &root, node);
            } break;
            case TT_VOID:
            case TT_PLUS:
